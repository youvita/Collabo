...
- (void)updateScrollViewDimensions
{
    ...
    switch (_alignment)
    {
        ...
        case SwipeViewAlignmentEdge:
        {
            if (_vertical)
            {
                frame = CGRectMake(-8.0f, 0.0f, self.bounds.size.width, _itemSize.height * _itemsPerPage);
                contentSize.height = _itemSize.height * _numberOfItems - (self.bounds.size.height - frame.size.height);
            }
            else
            {
                frame = CGRectMake(-8.0f, 0.0f, _itemSize.width * _itemsPerPage, self.bounds.size.height);
...
- (void)updateItemSizeAndCount
{
    ...
    {
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 67, 90)];
...
- (void)loadUnloadViews
{
    ...
    if (itemWidth)
    {
        ...
        NSMutableArray *visibleIndices = [[NSMutableArray alloc]init];
